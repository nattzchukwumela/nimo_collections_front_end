
import { Navbar } from "../../component/Navbar/Navbar";
import { Footer } from "../../component/Footer/Footer";
import { Aside } from "../../component/Aside/Aside";
import shots from '../../assets/products/men/hood.jpg';
import '../index_page.css';

export function Men() {
    return (<>
        <Navbar />
        <main className="main-men-section">
            <Aside />
            <section className='items-section'>
                <section className='product-row sports-section'>
                    <h2>Sport Clothings</h2>
                    <div className='men-section-container'>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                    </div>
                </section>

                <section className='product-row formal-section'>
                    <h2>Formal clothing</h2>
                    <div className='men-section-container'>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                    </div>
                </section>
                <section className='product-row native-section'>
                    <h2>Native Clothing</h2>
                    <div className='men-section-container'>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                    </div>
                </section>
                <section className='product-row t-shirt-sweat-shirt-section'>
                    <h2>T-shirt & Sweatshirts Clothing</h2>
                    <div className='men-section-container'>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                    </div>
                </section>
                <section className='product-row t-shirt-sweat-shirt-section'>
                    <h2>Long sleeves Clothing</h2>
                    <div className='men-section-container'>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                        <div className='men-section-container-item'>
                            <img src={shots} alt="" />
                            <h3 className="product-name">Hoodie</h3>
                            <p className='product-price'>£400</p>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.</p>
                            <button>Add to cart</button>
                        </div>
                    </div>
                </section>
            </section>
        </main>
        <Footer />
    </>)
}